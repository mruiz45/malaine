...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (adminProfileError) {
      console.error('Error fetching admin profile for performing user (demote):', adminProfileError.message);
      return res.status(500).json({ message: 'Error verifying admin status.' });
    }
    
    if (!adminProfile || !adminProfile.is_admin) {
      return res.status(403).json({ message: 'Forbidden: Only admins can demote users' });
    }
    ...
    // Prevent admin from demoting themselves via this endpoint if it's not desired
    // if (performingUser.id === targetUserId) {
    //   return res.status(400).json({ message: 'Cannot demote yourself via this endpoint.' });
    // }
    ...
    if (updateError) {
      ...
      return res.status(500).json({ message: 'Failed to demote user', error: updateError.message });
    }
    
    return res.status(200).json({ success: true, message: `User ${targetUserId} demoted from admin.` });

  } catch (error: any) {
    console.error('Error in demoteFromAdmin API (outer catch):', error);
