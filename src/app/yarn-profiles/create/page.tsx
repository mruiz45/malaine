...
import { createYarnProfile, validateYarnProfileData } from '@/services/yarnProfileService';
import type { CreateYarnProfile, FiberContent, YarnWeightCategory } from '@/types/yarn';
...
export default function CreateYarnProfilePage() {
  const { t } = useTranslation();
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState<{ [key: string]: string }>({});

  const [formData, setFormData] = useState<CreateYarnProfile>({
    yarn_name: '',
    brand_name: '',
    fiber_content: [],
  ...
  const handleInputChange = (field: keyof CreateYarnProfile, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const cleanedData: CreateYarnProfile = {
      ...
      await createYarnProfile(cleanedData);
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-2xl font-bold text-gray-900">{t('yarn.form.title_create')}</h1>
            <p className="text-gray-600 mt-1">{t('yarn.form.description_create')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex justify-end space-x-4">
              ...
              >
                {loading ? t('yarn.form.creating') : t('yarn.form.create_button')}
              </button>
            </div>
